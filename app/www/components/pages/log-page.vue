                ...
                <md-card class="md-layout-item md-size-50 md-small-size-100">
                    ...
                    <md-card-content class="md-layout md-gutter">
                        <div class="md-layout-item md-small-size-100">
                            ...
                            />
                            <md-dialog-prompt
                                :md-active.sync="exportLogDialog"
                                v-model="exportLogMail"
                                :md-title="translated.EXPORT_LOG"
                                :md-content="translated.EXPORT_LOG_TEXT"
                                :md-confirm-text="translated.SEND"
                                :md-cancel-text="translated.CANCEL"
                                @md-cancel="exportLogDialog = false; exportLogMail=''"
                                @md-confirm="exportLogDialog = false; exportLog();"
                            />
                            <md-button class="md-raised md-accent log-delete-btn" @click="deleteLogDialog = true" v-if="log.id">{{ translated.DELETE}}</md-button>
                            <md-button class="md-raised md-primary log-export-btn" @click="exportLogDialog = true" v-if="log.id && log.autogenerated">{{ translated.EXPORT }}</md-button>
                        </div>
                    </md-card-content>
                    <md-card-content>
    ...
    export default {
        ...
        methods: {
            ...
            exportLog() {
                ...
                http.sendRequest('get', 'logexport', {
                    akey: storage.getValue('akey'),
                    token: storage.getValue('token'),
                    email: self.exportLogMail,
                    id: self.log.id
                }, true, err => {
                    self.exportLogMail = '';
                    if (!err) {
                        self.$refs.snackbar.setMessage('LOG_EXPORTED', false, 'success');
                    } else {
                        self.$refs.snackbar.setMessage('LOG_EXPORTED_ERROR', false, 'error');
                    }
                });
            },
